...
function GalleryPart({ images, name, text, delayInGallery }) {
    AOS.init();
  ...
  return (
    <div className="GalleryPart">
      <div
        className="GalleryPart_container"
        data-aos="fade-up"
        data-aos-delay={delayInGallery}
      >
      ...
      <div className={`slideshow_container ${name}Slide`}>
          ...
          {img_array.map((imag, idx) => {
            return (
              <div key={uuidv4()} className={`mySlides fading ${name}`}>
                <div className="numbertext">{`${idx + 1}/${
                  img_array.length
                }`}</div>
        ...
        <a onClick={() => plusSlides(-1)} className="prev">
          &#10094;
        </a>
        <a onClick={() => plusSlides(1)} className="next">
          &#10095;
        </a>
      </div>
    </div>
